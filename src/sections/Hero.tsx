...
export default function Hero() {
  const { displayed, done } = useTypewriter('O in one eye\nX in the other')
    ...
    <section id="top" className="relative flex min-h-screen items-center overflow-hidden px-5 pb-16 pt-28 sm:px-8 lg:px-12 lg:pt-24">
      ...
      <div className="relative z-10 mx-auto grid w-full max-w-[1440px] items-center gap-12 lg:grid-cols-[.9fr_1.1fr] lg:gap-8">
        <motion.div variants={parent} initial="hidden" animate="visible" className="relative z-10 max-w-2xl">
          <motion.p variants={child} className="section-kicker">SOLANA NIGHT ENTITY</motion.p>
          ...
          <motion.p variants={child} className="mt-7 max-w-xl text-sm leading-6 text-muted sm:text-base sm:leading-7">A black low poly cat moving through neon corridors and the quiet side of Solana</motion.p>
          <motion.div variants={child} className="mt-9 flex flex-col gap-3 sm:flex-row">
            <a href="#about" className="angular-button group bg-white text-black">ENTER THE NIGHT <ArrowDownRight size={16} className="transition-transform group-hover:translate-x-1 group-hover:translate-y-1" /></a>
            <a href="#tokenomics" className="angular-button group border border-white/15 bg-white/[.04] text-white">VIEW TOKENOMICS <ArrowDownRight size={16} className="transition-transform group-hover:translate-x-1 group-hover:translate-y-1" /></a>
          </motion.div>
        </motion.div>
        <motion.div initial={{ opacity: 0, scale: .95 }} animate={{ opacity: 1, scale: 1 }} transition={{ duration: 1, delay: .25, ease: [0.22, 1, 0.36, 1] }}
          ...
          <motion.div animate={{ x: offset.x, y: offset.y }} transition={{ type: 'spring', stiffness: 80, damping: 18 }} className="hero-frame relative aspect-[4/5] w-full">
            ...
            <span className="ambient-label -left-2 top-[12%]">O // X</span><span className="ambient-label -right-2 top-[34%]">LOW POLY ENTITY</span><span className="ambient-label bottom-[8%] left-[5%]">SOLANA NIGHT</span>
          </motion.div>
