        ...
        <div className="grid items-center gap-12 lg:grid-cols-[1.03fr_.97fr] lg:gap-20">
          <motion.div initial={{ opacity: 0, x: -35 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true, amount: .25 }} transition={{ duration: .8, ease: [0.22, 1, 0.36, 1] }} className="relative">
            <div className="about-frame relative aspect-[4/5] overflow-hidden border border-white/10">
              ...
              <span className="absolute bottom-5 left-5 text-[9px] tracking-[.23em] text-white/70">SHELTER FILE // OX 01</span>
            </div>
          ...
          <motion.div initial={{ opacity: 0, y: 28 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true, amount: .3 }} transition={{ duration: .8, delay: .1, ease: [0.22, 1, 0.36, 1] }}>
            <p className="section-kicker">THE CAT BEHIND THE SIGNAL</p>
            <h2 className="mt-5 max-w-xl text-4xl font-medium leading-[1.02] tracking-[-.05em] sm:text-5xl lg:text-6xl">One eye gone<br />The spark stayed</h2>
            <div className="mt-8 max-w-xl space-y-4 text-sm leading-7 text-muted sm:text-base">
              <p>Oxmew came from a shelter built for cats the world tends to overlook</p>
              <p>One eye was gone, but the mischief never left</p>
              <p>The Solana community and its degens gave him a new life, a louder home and every reason to keep running</p>
              <p className="text-white">Now O and X carry one promise that no cat should be defined by what it has lost</p>
            </div>
            <div className="mt-9 flex flex-wrap gap-2">{['RESCUE CAT', 'STILL MISCHIEVOUS', 'O X IDENTITY', 'SOLANA RAISED'].map(tag => <span key={tag} className="border border-white/10 bg-white/[.03] px-3 py-2 text-[9px] tracking-[.2em] text-white/70">{tag}</span>)}</div>
          </motion.div>
        </div>
        <SceneSelector />
      </div>
