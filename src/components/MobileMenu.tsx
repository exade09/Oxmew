      ...
      {open && (
        <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} transition={{ duration: .35 }}
          ...
          <nav className="relative mt-16 flex flex-col border-y border-white/10 py-7" aria-label="Mobile navigation">
            {[{ label: 'Story', href: '#about' }, { label: 'Impact', href: '#impact' }].map((item, index) => (
              <motion.a key={item.label} initial={{ opacity: 0, x: -24 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: .1 + index * .08 }}
                href={item.href} onClick={close} className="py-4 text-4xl font-medium tracking-[-.04em] text-white transition hover:text-green focus-visible:outline-none focus-visible:text-pink">{item.label}</motion.a>
            ))}
          </nav>
          <div className="relative mt-auto"><p className="mb-4 text-[10px] tracking-[.26em] text-muted">JOIN THE RESCUE SIGNAL</p><SocialIcons mobile onNavigate={close} /></div>
        </motion.div>
      )}
    </AnimatePresence>
  )
